    ...
    <div className="flex gap-6 h-full min-h-screen pb-24 text-foreground">
      ...
      <div className="flex-1 space-y-6">
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-7 h-auto">
          {/* Recent Activity / Team Presence */}
          <div className="col-span-4 space-y-6 flex flex-col">
            <Card className="glass bg-white/[0.02] border-white/[0.08]">
              <CardHeader className="pb-3 border-b border-white/[0.06]">
                <CardTitle className="text-sm font-bold text-white">Team Directory & Presence</CardTitle>
              </CardHeader>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                  {employees.length === 0 ? (
                    ...
                    employees.slice(0, 6).map((emp: any) => (
                      <div key={emp.id} className="p-3 rounded-xl border border-white/[0.06] bg-white/[0.02] flex items-center gap-3">
                        <Avatar className="h-8 w-8">
                          <AvatarImage src={emp.avatar} />
                          <AvatarFallback className="text-xs">{emp.fullName?.substring(0, 2).toUpperCase() || emp.name?.substring(0, 2).toUpperCase() || "EM"}</AvatarFallback>
                        </Avatar>
                        <div className="min-w-0 flex-1">
                          <p className="text-xs font-bold text-white truncate">{emp.fullName || emp.name || "Employee"}</p>
                          <p className="text-[10px] text-white/40 truncate">{emp.designation || emp.role || "Team Member"}</p>
                        </div>
                      </div>
                    ))
                  )}
            ...
            {/* TEAM SHOUTOUTS */}
            <Card className="bg-card border border-border shadow-sm flex-1 flex flex-col">
