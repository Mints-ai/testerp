...
export default function SecureMail() {
  ...
  return (
    <div className="h-[calc(100vh-120px)] flex bg-[#121813]/40 backdrop-blur-xl border border-white/[0.08] rounded-2xl overflow-hidden text-foreground">
      ...
      <div className="w-56 bg-white/[0.01] border-r border-white/[0.06] flex flex-col p-4 shrink-0">
        <Dialog open={isComposeOpen} onOpenChange={setIsComposeOpen}>
          ...
          />
          <DialogContent className="sm:max-w-[500px] bg-[#121813] border border-white/[0.08] text-foreground p-6 rounded-2xl shadow-xl">
            <DialogHeader>
            ...
            <form onSubmit={handleSendMail} className="space-y-4 py-3">
              <div className="space-y-1.5 relative">
                <label className="text-[10px] font-bold text-foreground/50 uppercase tracking-wider block">Secure Recipient</label>
                  ...
                  <div className="flex items-center justify-between p-3 bg-blue-500/10 border border-blue-500/20 rounded-xl">
                    <div className="flex items-center gap-2">
                      ...
                      <div className="text-xs min-w-0">
                        ...
                        <p className="text-[10px] text-foreground/45 truncate">{employees.find(emp => emp.id === composeTo)?.email}</p>
                      </div>
                    </div>
                    <button
                      type="button"
                          ...
                          employees.filter(emp => {
                            ...
                            >
                              ...
                              <div className="text-left min-w-0">
                                ...
                                <p className="text-[9px] text-foreground/40 truncate">{emp.email}</p>
                              </div>
                            </div>
                          ))
                        )}
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="col-span-2 space-y-1.5">
                  <label className="text-[10px] font-bold text-foreground/50 uppercase tracking-wider">Subject</label>
                  <Input
                    required
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-foreground/50 uppercase tracking-wider">Priority</label>
                  <select
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-bold text-foreground/50 uppercase tracking-wider">Memo details</label>
                <textarea
                  required
                  rows={4}
                  ...
                  className="w-full border border-border rounded-xl p-3 text-xs focus:border-blue-500/60 focus:ring-0 bg-[#121813] text-foreground placeholder:text-foreground/20"
                />
              </div>

              {/* Secure Document Attachment Linker */}
              <div className="space-y-2 border-t border-white/[0.05] pt-3">
                <label className="text-[10px] font-bold text-foreground/50 uppercase tracking-wider block">Secure Document Attachments</label>
                ...
                <div className="flex gap-2">
                  <Input
                    ...
                    className="glass-input h-8 text-[10px] border-border placeholder:text-foreground/20 focus:border-blue-500/60 focus:ring-0 flex-1"
                  />
                  <Input
        ...
        <div className="space-y-1 flex-1">
          <div className="text-[9px] font-bold uppercase tracking-wider text-foreground/30 mb-2 px-3">secure Folders</div>
          {[
              ...
              <button
                ...
                className={cn(
                  ...
                  priorityFilter === p.id 
                    ...
                    : "bg-white/[0.02] text-foreground/40 border-white/[0.05] hover:text-foreground/60"
                )}
              >
                {p.label}
              </button>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0 space-y-1">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[9px] text-foreground/25 shrink-0 font-mono">
                    ...
                    <p className="text-[10px] text-foreground/30 truncate leading-relaxed">
                      {mail.body}
                    </p>
                    ...
                    <div className="flex items-center gap-2 pt-1.5">
                      ...
                      {mail.priority === "low" && (
                        <Badge variant="outline" className="bg-slate-500/10 text-slate-400 border-border/30 text-[8px] py-0 px-1 font-bold uppercase tracking-wider">Low</Badge>
                      )}
                      <button
                        onClick={(e) => handleToggleStar(e, mail)}
                        className={cn(
                          "ml-auto cursor-pointer p-0.5 hover:bg-muted/40 rounded-md",
      ...
      <div className="flex-1 flex flex-col bg-white/[0.002]">
        {selectedMail ? (
            ...
            <div className="p-4 border-b border-white/[0.06] flex items-center justify-between shrink-0">
              <div className="flex items-center gap-2.5">
                <button
                  ...
                  className="btn-ghost h-8 py-0 px-3.5 text-[10px] uppercase font-bold flex items-center gap-1 cursor-pointer border-border hover:text-foreground"
                >
                ...
                <button
                  ...
                  className="btn-ghost h-8 py-0 px-3 text-[10px] uppercase font-bold flex items-center gap-1 cursor-pointer border-border hover:text-foreground"
                >
                ...
                {activeFolder === "trash" ? (
                  <button
                    ...
                    className="btn-ghost h-8 py-0 px-3 text-[10px] uppercase font-bold flex items-center gap-1 cursor-pointer border-border text-emerald-400 hover:text-emerald-300"
                  >
              ...
              <button
                ...
                className="btn-ghost h-8 py-0 px-3 text-[10px] uppercase font-bold flex items-center gap-1 cursor-pointer border-border text-rose-400 hover:text-rose-300"
              >
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6 scrollbar-hide">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0 space-y-1">
                  ...
                  <p className="text-[10px] text-foreground/40 font-semibold font-mono">
              ...
              {/* Body */}
              <div className="text-xs text-foreground/70 leading-relaxed whitespace-pre-wrap font-medium p-4 rounded-2xl bg-white/[0.01] border border-white/[0.04]">
                {selectedMail.body}
              </div>
              ...
              {selectedMail.attachments && selectedMail.attachments.length > 0 && (
                <div className="space-y-2 border-t border-white/[0.05] pt-4">
                  <h4 className="text-[10px] font-bold text-foreground/40 uppercase tracking-wider">Secure Attachments</h4>
                    ...
                    {selectedMail.attachments.map((att: any, index: number) => (
                      ...
                      >
                        <div className="flex items-center gap-2.5 min-w-0">
                          ...
                          <div className="text-left min-w-0">
                            <p className="text-xs font-bold text-foreground truncate group-hover:text-blue-300 transition-colors">{att.name}</p>
                            <p className="text-[9px] text-foreground/30 truncate">{att.url}</p>
                          </div>
                        </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-foreground/20 p-8 text-center">
            ...
            <p className="text-[10px] text-foreground/30 max-w-xs mt-1">Select an email memo from the column list to inspect its secure internal contents.</p>
          </div>
        )}
      </div>
