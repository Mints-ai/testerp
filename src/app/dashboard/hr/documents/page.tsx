...
type FilterStatus = "all" | "expired" | "critical" | "warning" | "notice" | "valid";
...
type CardSpec = {
  ...
  color: string;
  click: Exclude<FilterStatus, "all">;
};
...
export default function DocumentExpiryTracker() {
  ...
  useEffect(() => {
    const unsub = onSnapshot(collection(db, "employees"), snap => {
      setEmployees(snap.docs.map(d => ({ id: d.id, ...(d.data() as Partial<Employee>) })) as Employee[]);
    });
    return () => unsub();
  }, []);
  ...
  useEffect(() => {
    const unsub = onSnapshot(
      ...
      snap => {
        setDocs(snap.docs.map(d => ({ id: d.id, ...(d.data() as Partial<DocRecord>) } as DocRecord)));
        setLoading(false);
      }
    );
    return () => unsub();
    ...
    >
      <div className="space-y-6 pb-12">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          ] as CardSpec[]).map(card => {
            const Icon = card.icon;
            return (
              <button
                key={card.label}
                onClick={() => setFilterStatus(filterStatus === card.click ? "all" : card.click)}
