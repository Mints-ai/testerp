        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h1 className="text-xl font-bold tracking-tight flex items-center gap-2">
              <Clock className="h-5 w-5 text-primary fill-indigo-500/10" />
              ...
              <div className="grid gap-4 md:grid-cols-3">
                <Card className="bg-card border border-border shadow-sm rounded-lg border-border">
                  <CardHeader className="flex flex-row items-center justify-between pb-2">
                    <CardTitle className="text-xs font-bold text-foreground/50 uppercase tracking-wider">Total Team Monitored</CardTitle>
                    <Users className="h-4 w-4 text-accent" />
                  </CardHeader>
                  <CardContent>
                    <div className="text-2xl font-bold">{teamCapacity.length}</div>
                    <p className="text-xs text-foreground/40 mt-1">Actively tracking allocations</p>
                  </CardContent>
                </Card>

                <Card className="bg-card border border-border shadow-sm rounded-lg border-rose-500/20">
                  <CardHeader className="flex flex-row items-center justify-between pb-2">
                    <CardTitle className="text-xs font-bold text-rose-400 uppercase tracking-wider">Over-Allocated</CardTitle>
                    <AlertTriangle className="h-4 w-4 text-rose-400" />
                  </CardHeader>
                  <CardContent>
                    <div className="text-2xl font-bold text-rose-400">{overbookedCount}</div>
                    <p className="text-xs text-foreground/40 mt-1">Exceeding 85% capacity threshold</p>
                  </CardContent>
                </Card>

                <Card className="bg-card border border-border shadow-sm rounded-lg border-emerald-500/20">
                  <CardHeader className="flex flex-row items-center justify-between pb-2">
                    <CardTitle className="text-xs font-bold text-accent uppercase tracking-wider">Available Bandwidth</CardTitle>
                    <CheckCircle2 className="h-4 w-4 text-accent" />
                  </CardHeader>
                  <CardContent>
                    <div className="text-2xl font-bold text-accent">{availableCount}</div>
                    <p className="text-xs text-foreground/40 mt-1">Ready for assignment</p>
                  </CardContent>
                </Card>
              </div>
              ...
              <Card className="bg-card border border-border shadow-sm border-border">
                <CardHeader>
                  <CardTitle className="text-sm font-bold text-foreground flex items-center gap-2">
                    <TrendingUp className="h-4 w-4 text-accent" /> Team Utilization Heatmap
                  </CardTitle>
                  <CardDescription className="text-xs text-foreground/40">Calculated allocations against standard 40-hour work week capacities.</CardDescription>
                </CardHeader>
                <CardContent>
                  {loading ? (
                    ...
                    <div className="space-y-4">
                      {teamCapacity.map((member, i) => (
                        <div 
                          key={member.userId} 
                          className="flex flex-col sm:flex-row sm:items-center gap-4 p-4 rounded-xl border border-border hover: transition-colors"
                        >
                          <div className="flex items-center gap-4 w-full sm:w-1/3">
                            <Avatar className="h-10 w-10 border border-border/30 shadow-sm">
                              <AvatarImage src={member.avatar} />
                              <AvatarFallback className="bg-primary/30 text-foreground font-bold text-xs">
                          ...
                          <div className="flex items-center justify-between sm:justify-end gap-6 w-full sm:w-1/3 mt-2 sm:mt-0">
                            <div className="text-right">
                              ...
                              <p className="text-xs text-foreground/40 mt-0.5">{member.activeTasks} active tasks</p>
                            </div>
                            <Badge variant="outline" className={`${getStatusColor(member.status)} text-xs w-24 justify-center py-0.5 font-bold uppercase`}>
                              {member.status}
                            </Badge>
                          </div>
                        </div>
            ...
            >
              ...
              <Card className="bg-card border border-border shadow-sm border-border">
                <CardContent className="p-5 flex flex-col md:flex-row gap-4 items-end">
                  <div className="flex-1 space-y-1.5">
                    <label className="text-xs font-bold text-foreground/50 uppercase tracking-wider">Employee Reference</label>
                    <select
                  ...
                  <div className="flex-1 space-y-1.5">
                    <label className="text-xs font-bold text-foreground/50 uppercase tracking-wider">Target Week</label>
                    <select
                      value={selectedWeek}
                  ...
                  <div className="text-right">
                    <div className="text-xs font-bold text-foreground/50 uppercase tracking-wider">Total Matrix Hours</div>
                    <div className="text-xl font-black text-accent mt-1">{getGrandTotal().toFixed(1)} hrs</div>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="bg-card border border-border shadow-sm border-border overflow-hidden">
                <CardHeader className="pb-3 border-b border-border flex flex-row items-center justify-between">
                  <div>
                    <CardTitle className="text-sm font-bold text-foreground flex items-center gap-2">
                      <FileSpreadsheet className="h-4 w-4 text-accent" /> Unified Weekly Matrix Grid
                    </CardTitle>
                    <CardDescription className="text-xs text-foreground/40">Log and calculate project hours dynamically across columns.</CardDescription>
                  </div>
                  <button
                    onClick={addTimesheetRow}
                    className="flex items-center gap-1 bg-primary/10 hover:bg-primary/20 text-primary border border-primary/20 rounded-xl px-3.5 py-1.5 text-xs font-bold transition-all shadow-sm"
                  >
                  ...
                  <table className="w-full border-collapse text-left min-w-[800px]">
                    <thead>
                      <tr className="border-b border-border">
                        <th className="p-3 text-xs font-bold text-foreground/50 uppercase tracking-wider w-64">Client & Project</th>
                        <th className="p-3 text-xs font-bold text-foreground/50 uppercase tracking-wider text-center w-20">Mon</th>
                        <th className="p-3 text-xs font-bold text-foreground/50 uppercase tracking-wider text-center w-20">Tue</th>
                        <th className="p-3 text-xs font-bold text-foreground/50 uppercase tracking-wider text-center w-20">Wed</th>
                        <th className="p-3 text-xs font-bold text-foreground/50 uppercase tracking-wider text-center w-20">Thu</th>
                        <th className="p-3 text-xs font-bold text-foreground/50 uppercase tracking-wider text-center w-20">Fri</th>
                        <th className="p-3 text-xs font-bold text-foreground/50 uppercase tracking-wider text-center w-20">Sat</th>
                        <th className="p-3 text-xs font-bold text-foreground/50 uppercase tracking-wider text-center w-20">Sun</th>
                        <th className="p-3 text-xs font-bold text-foreground/50 uppercase tracking-wider text-center w-24">Row Total</th>
                        <th className="p-3 text-xs font-bold text-foreground/50 uppercase tracking-wider text-center w-16">Action</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-white/[0.04]">
                        ...
                        <tr key={row.id} className="hover: transition-all">
                          <td className="p-3">
                            <div className="space-y-1">
                              ...
                              {row.clientId && (
                                <p className="text-xs text-foreground/40 font-bold px-1">{row.clientId}</p>
                              )}
                            </div>
                          </td>
                          {(["mon", "tue", "wed", "thu", "fri", "sat", "sun"] as const).map((day) => (
                            <td key={day} className="p-3 text-center">
                              <input
                                ...
                                className="w-14 border border-border/30 focus:border-primary focus: rounded-xl py-1.5 text-center text-xs font-bold focus:outline-none focus:ring-0 placeholder:text-foreground/10"
                              />
                            </td>
                          ))}
                      ...
                      <tr className="border-t-2 border-border font-black text-xs">
                        <td className="p-3 text-xs font-bold text-foreground/50 uppercase tracking-widest pl-4">Daily Totals</td>
                        {(["mon", "tue", "wed", "thu", "fri", "sat", "sun"] as const).map((day) => {
                          const colTot = getColumnTotal(day);
                          return (
                            <td key={day} className="p-3 text-center tabular-nums font-black text-accent">
                              {colTot > 0 ? `${colTot.toFixed(1)}h` : "—"}
                            </td>
                          );
                        })}
                        <td className="p-3 text-center tabular-nums text-accent">
                ...
                <div className="p-4 border-t border-border flex justify-between items-center">
                  <p className="text-xs text-foreground/35 font-bold uppercase tracking-wider">All project logged hours automatically sync to Global Ledger.</p>
                  <button
                    onClick={handleSubmitTimesheet}
                    disabled={isSubmitting || getGrandTotal() === 0}
                    className="flex items-center gap-1.5 bg-primary hover:bg-primary disabled:opacity-40 disabled:cursor-not-allowed text-foreground px-5 py-2 rounded-xl text-xs font-bold transition-all shadow-md shadow-indigo-600/10 hover:shadow-indigo-600/20"
                  >
                    {isSubmitting ? (
              ...
              <Card className="bg-card border border-border shadow-sm rounded-lg border-border overflow-hidden">
                <div className="p-6 border-b border-border flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                  <div>
                    <h3 className="text-sm font-bold text-foreground uppercase tracking-wider flex items-center gap-2">
                      <GanttChartSquare className="h-4 w-4 text-accent" /> Active Schedule Gantt & Milestones
                    </h3>
                    <p className="text-xs text-foreground/40 mt-1">Cross-project timelines, delivery milestones, and executive accountability indicators.</p>
                  </div>
                  <div className="flex items-center gap-4 text-xs font-bold text-foreground/40 tracking-wider">
                    ...
                    <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-primary/20 border border-primary/50" /> Pitch</span>
                    <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-primary/20 border border-primary/50" /> Completed</span>
                ...
                <div className="p-6 overflow-x-auto">
                  <div className="min-w-[800px] space-y-4">
                    ...
                    <div className="flex border-b border-border pb-2 text-xs font-bold uppercase tracking-wider text-foreground/30">
                      <div className="w-1/4">Project Deliverable</div>
                      <div className="w-3/4 flex relative">
                        <div className="w-1/4 text-center border-l border-border">May 2026</div>
                        <div className="w-1/4 text-center border-l border-border">June 2026</div>
                        <div className="w-1/4 text-center border-l border-border">July 2026</div>
                        <div className="w-1/4 text-center border-l border-border border-r border-border">August 2026</div>
                      </div>
                    </div>
                    ...
                    {projects.length === 0 ? (
                      ...
                      projects.map((proj) => {
                        ...
                        const barGradient = 
                          proj.status === 'completed' ? 'from-indigo-600 to-purple-600 border-primary/30' :
                          proj.status === 'pitch' ? 'from-blue-600 to-cyan-600 border-primary/30' :
                        ...
                        return (
                          <div key={proj.id} className="flex items-center py-3 border-b border-border group/row hover: transition-colors rounded-xl px-2">
                            {/* Project Name and Category Info */}
                            <div className="w-1/4 pr-4">
                              <span className="font-bold text-xs text-foreground group-hover/row:text-accent transition-colors block truncate">{proj.name}</span>
                              <span className="text-xs font-bold text-foreground/40 uppercase tracking-wider block mt-0.5">{proj.serviceType || "Deliverable"}</span>
                            </div>
                            ...
                            <div className="w-3/4 h-12 flex items-center relative select-none">
                              ...
                              <div className="absolute inset-0 flex pointer-events-none">
                                ...
                                <div className="w-1/4 border-l border-border" />
                                <div className="w-1/4 border-l border-border" />
                                <div className="w-1/4 border-l border-border border-r border-border" />
                              </div>
                              ...
                              >
                                <span className="text-xs font-bold text-foreground uppercase tracking-wider truncate">{proj.status || "active"}</span>
                                <span className="text-xs font-mono font-bold text-foreground/80">{progressVal}%</span>

                                {/* Quick Hover Inspector Floating Card (Glass Tooltip) */}
                                <div className="absolute left-1/2 -translate-x-1/2 bottom-10 w-64 bg-background/95 border border-border rounded-2xl p-4 shadow-xl pointer-events-none opacity-0 group-hover/bar:opacity-100 transition-all duration-300 translate-y-2 group-hover/bar:translate-y-0 z-50 text-foreground">
                                  <div className="space-y-3">
                                    <div className="flex justify-between items-start">
                                      <div className="truncate pr-2">
                                        <p className="text-xs font-bold text-foreground uppercase tracking-wider truncate">{proj.name}</p>
                                        <p className="text-xs font-semibold text-accent uppercase tracking-widest mt-0.5 truncate">{proj.serviceType}</p>
                                      </div>
                                      <span className="text-xs font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-muted/80 shrink-0">{proj.status}</span>
                                    </div>

                                    {proj.description && (
                                      <p className="text-xs text-foreground/60 leading-normal line-clamp-2">{proj.description}</p>
                                    )}

                                    <div className="border-t border-border pt-2.5 grid grid-cols-2 gap-2">
                                      <div>
                                        <p className="text-xs font-bold text-foreground/35 uppercase tracking-wider">Lead Director</p>
                                        <div className="flex items-center gap-1 mt-1">
                                          <div className="w-4 h-4 rounded-full bg-primary/20 border border-primary/30 flex items-center justify-center text-xs font-bold text-primary shrink-0">
                                            {getInitials(mgrName)}
                                          </div>
                                          <span className="text-xs font-semibold text-foreground/80 truncate max-w-[80px]">{mgrName}</span>
                                        </div>
                                      </div>
                                      <div>
                                        <p className="text-xs font-bold text-foreground/35 uppercase tracking-wider">Remaining</p>
                                        <span className="text-xs font-semibold text-accent block mt-1 font-mono">{remainingText}</span>
                                      </div>
                                    </div>

                                    <div className="border-t border-border pt-2.5 flex justify-between items-center text-xs font-bold text-foreground/40 uppercase tracking-wider">
